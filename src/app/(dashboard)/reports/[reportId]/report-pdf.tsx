...
export function ReportPdf({ data }: { data: ReportPdfData }) {
  ...
  pages.push(
    <Page key="main" size="LETTER" style={styles.page}>
      <Text style={styles.title}>{BRAND.artifactSummary}</Text>
      <Text style={styles.subtitle}>
        {BRAND.artifact} — {data.filename} — Generated{" "}
      ...
      <Text style={styles.footer} fixed>
        ...
        {data.verificationId
          ...
          : ""}
        {"\n"}
        This record confirms only the listed verification checks performed on the
        identified document version. — Page {"$page"} of {"$pages"}
      </Text>
    </Page>
  );
